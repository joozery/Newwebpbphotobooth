...
const AdminPromotionSlides = () => {
  ...
  const fetchSlides = async () => {
    try {
      setLoading(true);
      const data = await promotionSlideService.getAllSlides();
      setSlides(data);
    } catch (error) {
      console.error('Error fetching promotion slides:', error);
      toast.error('ไม่สามารถโหลดข้อมูล Promotion Slides ได้');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingSlide) {
        await promotionSlideService.updateSlide(editingSlide.id, selectedFile);
        toast.success('อัพเดท Promotion Slide สำเร็จ');
      } else {
        await promotionSlideService.createSlide(selectedFile);
        toast.success('สร้าง Promotion Slide สำเร็จ');
      }
      ...
      console.error('Error saving promotion slide:', error);
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('คุณแน่ใจหรือไม่ที่จะลบ Promotion Slide นี้?')) {
      return;
    }

    try {
      await promotionSlideService.deleteSlide(id);
      toast.success('ลบ Promotion Slide สำเร็จ');
      fetchSlides();
    } catch (error) {
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      await promotionSlideService.toggleSlideStatus(id);
  ...
  const handleReorder = async (id, direction) => {
    ...
    try {
      await Promise.all([
        promotionSlideService.updateSlideOrder(id, { slide_order: targetSlide.slide_order }),
        promotionSlideService.updateSlideOrder(targetSlide.id, { slide_order: currentSlide.slide_order })
      ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">จัดการ Promotion Slides</h1>
          <p className="text-gray-600">อัพโหลดรูปภาพสำหรับ Promotion Section ของเว็บไซต์</p>
        </div>
        <button
      ...
      {slides.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">ยังไม่มี Promotion Slides</h3>
          <p className="text-gray-600 mb-4">เริ่มต้นด้วยการอัพโหลดรูปภาพโปรโมชั่นแรกของคุณ</p>
          <button
            onClick={openCreateModal}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
            <h2 className="text-xl font-bold mb-4">
              {editingSlide ? 'แก้ไข Promotion Slide' : 'อัพโหลดรูปภาพใหม่'}
            </h2>
...
export default AdminPromotionSlides; 